...
public class CaptureConfigurationPanel extends JPanel{
	...
	private void value() {
		...
		//마우스 표시 옵션
		System.out.println(conf.getGuide() == CaptureConfiguration.SHOW);
	...
	private void event() {
		...
		components.get("pause", JCheckBox.class).addChangeListener(e->{
			conf.setBackground(
				components.isSelected("pause") ? 
					CaptureConfiguration.PAUSE : CaptureConfiguration.TRANSPARENT
			);
		});
		...
		components.get("pixel", JCheckBox.class).addChangeListener(e->{
			conf.setPixel(
				components.isSelected("pixel") ?
					CaptureConfiguration.SHOW : CaptureConfiguration.HIDE
			);
		});
